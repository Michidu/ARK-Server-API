...
struct TAngle
{
	...
	{
		Degrees = vec_t(
			((int(*(volatile vec_t *)&Degrees * ((1 << 30) / 360.0)) + (1 << 29) - 1 & (1 << 30) - 1) - (1 << 29) + 1) * (360.f /
				(1 << 30)));
		return *this;
	}
...
template <class T>
double ToRadians(const TAngle<T>& deg)
{
	return double(deg.Degrees * (PI / 180.0));
}

template <class T>
TAngle<T> ToDegrees(double rad)
{
	return TAngle<T>(double(rad * (180.0 / PI)));
}

template <class T>
double cos(const TAngle<T>& deg)
{
	return cos(ToRadians(deg));
}

template <class T>
double sin(const TAngle<T>& deg)
{
	return sin(ToRadians(deg));
}

template <class T>
double tan(const TAngle<T>& deg)
{
	return tan(ToRadians(deg));
}

template <class T>
TAngle<T> fabs(const TAngle<T>& deg)
{
	return TAngle<T>(fabs(deg.Degrees));
}

template <class T>
TAngle<T> vectoyaw(const TVector2<T>& vec)
{
	return atan2(vec.Y, vec.X) * (180.0 / PI);
}

template <class T>
TAngle<T> vectoyaw(const TVector3<T>& vec)
{
	return atan2(vec.Y, vec.X) * (180.0 / PI);
}
...
struct TRotator
{
	...
	{
		return fabs(Pitch - other.Pitch) < Angle(EQUAL_EPSILON) && fabs(Yaw - other.Yaw) < Angle(EQUAL_EPSILON) &&
			fabs(Roll - other.Roll) < Angle(EQUAL_EPSILON);
	}
...
template <class T>
TVector3<T>::TVector3(const TRotator<T>& rot)
	: X(cos(rot.Pitch) * cos(rot.Yaw)), Y(cos(rot.Pitch) * sin(rot.Yaw)), Z(-sin(rot.Pitch))
{
}

template <class T>
TMatrix3x3<T>::TMatrix3x3(const TVector3<T>& axis, TAngle<T> degrees)
{
