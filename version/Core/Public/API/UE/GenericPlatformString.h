...
struct FGenericPlatformString
{
	...
	struct TAreEncodingsCompatible
	{
		enum
		{
			Value = TIsFixedWidthEncoding<EncodingA>::Value && TIsFixedWidthEncoding<EncodingB>::Value && sizeof(EncodingA) ==
			sizeof(EncodingB)
		};
	};
	...
	static FORCEINLINE typename TEnableIf<
		...
		DestEncoding*
	>::Type Convert(DestEncoding* Dest, int32 DestSize, const SourceEncoding* Src, int32 SrcSize,
	                DestEncoding BogusChar = (DestEncoding)'?')
	{
		if (DestSize < SrcSize)
			return nullptr;

		return (DestEncoding*)memcpy(Dest, Src, SrcSize * sizeof(SourceEncoding)) +
			SrcSize;
	}
	 ...
	 */
	template <typename DestEncoding, typename SourceEncoding>
	static typename TEnableIf<TIsFixedWidthEncoding<SourceEncoding>::Value && TIsFixedWidthEncoding<DestEncoding>::Value,
	                          int32>::Type ConvertedLength(const SourceEncoding* Src, int32 SrcSize)
	{
		return SrcSize;
	}
};
